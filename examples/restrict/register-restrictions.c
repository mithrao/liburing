...
static int test_restrictions_empty(void)
{
    ...
    /* no whitelist - will deny every SQE */
	if (cqe->res != -EACCES) {
		fprintf(stderr, "write res: %d\n", cqe->res);
		return TEST_FAILED;
	}
...
static int test_restrictions_rings_not_disabled(void)
{
    ...
    /* need IORING_SETUP_R_DISABLED in arg_3 */
    ...
    ret = io_uring_register_restrictions(&ring, res, 1);
	if (ret != -EBADFD) {
        /* didn't setup IORING_SETUP_R_DISABLED, will fail */
...
static int test_restrictions_rings_disabled(void)
{
    ...
    /* didn't register whitelist, will fail */
